...
export function parseSchedule(input: string): string {
	...
	if (
		["hourly", "daily", "weekly", "monthly", "yearly", "minutely"].includes(
			clean,
		) ||
		clean.includes("*")
	) {
	...
	if (timePart) {
		...
		if (!parsed) {
			throw new Error(
				`[Stovetimer] Unsupported time format in schedule: "${input}"`,
			);
		}
		sysTime = parsed;
	}
	...
	const scheduleStr = sysDow
		? `${sysDow} ${sysDay} ${sysTime}`
		: `${sysDay} ${sysTime}`;
